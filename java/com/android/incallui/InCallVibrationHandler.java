...
public class InCallVibrationHandler extends Handler implements
    ...
    private static final int MSG_VIBRATE_50_SEC = 1;
    ...
    private static final String KEY_VIBRATE_50SECS = "incall_vibrate_50secs";
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            case MSG_VIBRATE_50_SEC:
                vibrate(70, 0, 0);
                sendEmptyMessageDelayed(MSG_VIBRATE_50_SEC, 60000);
                break;
        }
    }
    ...
    private void handleOutgoingCallVibration(DialerCall call) {
        ...
        if (mPrefs.getBoolean(KEY_VIBRATE_50SECS, false)) {
            start50SecondVibration(durationMillis);
        }
    }
    ...
    private void start50SecondVibration(long callDurationMillis) {
        callDurationMillis = callDurationMillis % 60000;
        Log.d(this, "vibrate start @" + callDurationMillis);
        removeMessages(MSG_VIBRATE_50_SEC);

        long timer;
        if (callDurationMillis > 50000) {
            // Schedule the alarm at the next minute + 50 secs
            timer = 50000 + 60000 - callDurationMillis;
        } else {
            // Schedule the alarm at the first 50 second mark
            timer = 50000 - callDurationMillis;
        }
        sendEmptyMessageDelayed(MSG_VIBRATE_50_SEC, timer);
    }
